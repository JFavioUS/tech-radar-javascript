...
const Sheet = function (sheetReference) {
  ...
  self.validate = function (callback) {
    var apiKeyEnabled = 'AIzaSyDgEokvTt4-XfKYkXV1WKIWJqvpHsIROtg' || process.env.API_KEY || false
  ...
  function processSheetData(sheetName, sheetResponse, createBlips, handleError) {
    ...
    self
      ...
      .then((r) => {
        createBlips(sheetResponse.result.properties.title, r.result.values, sheetNames)
      })
      .catch(handleError)
  }
